  ...
  for (const auto &[param, title, desc, icon] : visualToggles) {
    ...
    if (param == "AlertVolumeControl") {
      ...
      QObject::connect(manageCustomColorsBtn, &FrogPilotButtonsControl::buttonClicked, [=](int id) {
        ...
        QString currentColor = QString::fromStdString(params.get("CustomColors")).replace('_', ' ').toLower();
        ...
        if (id == 0) {
          ...
          if (!colorSchemeToDelete.isEmpty() && ConfirmationDialog::confirm(tr("Are you sure you want to delete the '%1' color scheme?").arg(colorSchemeToDelete), tr("Delete"), this)) {
            ...
            QString selectedColor = formatColorName(colorSchemeToDelete);
            for (const QFileInfo &dirInfo : dirList) {
          ...
          if (manageCustomColorsBtn->getButton(id)->text() == tr("CANCEL")) {
            ...
            if (!colorSchemeToDownload.isEmpty()) {
              QString convertedColorScheme = formatColorName(colorSchemeToDownload);
          ...
          if (!colorSchemeToSelect.isEmpty()) {
            params.put("CustomColors", formatColorName(colorSchemeToSelect).toStdString());
            manageCustomColorsBtn->setValue(colorSchemeToSelect);
            updateFrogPilotToggles();
          }
        }
      });
      QString customColors = QString::fromStdString(params.get("CustomColors")).replace('_', ' ').toLower();
      customColors[0] = customColors[0].toUpper();
      for (int i = 1; i < customColors.length(); ++i) {
        if (customColors[i - 1] == ' ' || customColors[i - 1] == '(') {
          customColors[i] = customColors[i].toUpper();
        }
      }
      manageCustomColorsBtn->setValue(customColors);
      ...
      QObject::connect(manageCustomIconsBtn, &FrogPilotButtonsControl::buttonClicked, [=](int id) {
        ...
        QString currentIcon = QString::fromStdString(params.get("CustomIcons")).replace('_', ' ').toLower();
        ...
        if (id == 0) {
          ...
          if (!iconPackToDelete.isEmpty() && ConfirmationDialog::confirm(tr("Are you sure you want to delete the '%1' icon pack?").arg(iconPackToDelete), tr("Delete"), this)) {
            ...
            QString selectedIcon = formatIconName(iconPackToDelete);
            for (const QFileInfo &dirInfo : dirList) {
          ...
          if (!iconPackToSelect.isEmpty()) {
            params.put("CustomIcons", formatIconName(iconPackToSelect).toStdString());
            manageCustomIconsBtn->setValue(iconPackToSelect);
            updateFrogPilotToggles();
          }
        }
      });
      QString customIcons = QString::fromStdString(params.get("CustomIcons")).replace('_', ' ').toLower();
      customIcons[0] = customIcons[0].toUpper();
      for (int i = 1; i < customIcons.length(); ++i) {
        if (customIcons[i - 1] == ' ' || customIcons[i - 1] == '(') {
          customIcons[i] = customIcons[i].toUpper();
        }
      }
      manageCustomIconsBtn->setValue(customIcons);
      ...
      QObject::connect(manageCustomSignalsBtn, &FrogPilotButtonsControl::buttonClicked, [=](int id) {
        ...
        QString currentSignal = QString::fromStdString(params.get("CustomSignals")).replace('_', ' ').toLower();
        ...
        for (const QFileInfo &dirInfo : dirList) {
          ...
          if (QDir(signalDir + "/signals").exists()) {
            ...
            availableSignals << formattedName;
          }
        }
        ...
        if (id == 0) {
          ...
          if (!signalPackToDelete.isEmpty() && ConfirmationDialog::confirm(tr("Are you sure you want to delete the '%1' signal pack?").arg(signalPackToDelete), tr("Delete"), this)) {
            ...
            QString selectedSignal = formatSignalName(signalPackToDelete);
            for (const QFileInfo &dirInfo : dirList) {
          ...
          if (manageCustomSignalsBtn->getButton(id)->text() == tr("CANCEL")) {
            ...
            if (!signalPackToDownload.isEmpty()) {
              QString convertedSignalPack = formatSignalName(signalPackToDownload);
          ...
          if (!signalPackToSelect.isEmpty()) {
            params.put("CustomSignals", formatSignalName(signalPackToSelect).toStdString());
            manageCustomSignalsBtn->setValue(signalPackToSelect);
            updateFrogPilotToggles();
          }
        }
      });
      QString customSignals = QString::fromStdString(params.get("CustomSignals")).replace('_', ' ').toLower();
      customSignals[0] = customSignals[0].toUpper();
      for (int i = 1; i < customSignals.length(); ++i) {
        if (customSignals[i - 1] == ' ' || customSignals[i - 1] == '(') {
          customSignals[i] = customSignals[i].toUpper();
        }
      }
      manageCustomSignalsBtn->setValue(customSignals);
      ...
      QObject::connect(manageCustomSoundsBtn, &FrogPilotButtonsControl::buttonClicked, [=](int id) {
        ...
        QString currentSound = QString::fromStdString(params.get("CustomSounds")).replace('_', ' ').toLower();
        ...
        if (id == 0) {
          ...
          if (!soundSchemeToDelete.isEmpty() && ConfirmationDialog::confirm(tr("Are you sure you want to delete the '%1' sound scheme?").arg(soundSchemeToDelete), tr("Delete"), this)) {
            ...
            QString selectedSound = formatSoundName(soundSchemeToDelete);
            for (const QFileInfo &dirInfo : dirList) {
          ...
          if (manageCustomSoundsBtn->getButton(id)->text() == tr("CANCEL")) {
            ...
            if (!soundSchemeToDownload.isEmpty()) {
              QString convertedSoundScheme = formatSoundName(soundSchemeToDownload);
          ...
          if (!soundSchemeToSelect.isEmpty()) {
            params.put("CustomSounds", formatSoundName(soundSchemeToSelect).toStdString());
      ...
      QString customSounds = QString::fromStdString(params.get("CustomSounds")).replace('_', ' ').toLower();
      customSounds[0] = customSounds[0].toUpper();
      for (int i = 1; i < customSounds.length(); ++i) {
        if (customSounds[i - 1] == ' ' || customSounds[i - 1] == '(') {
          customSounds[i] = customSounds[i].toUpper();
        }
      }
      manageCustomSoundsBtn->setValue(customSounds);
      ...
      QObject::connect(manageWheelIconsBtn, &FrogPilotButtonsControl::buttonClicked, [=](int id) {
        ...
        QString currentWheel = QString::fromStdString(params.get("WheelIcon")).replace('_', ' ').toLower();
        ...
        for (const QFileInfo &fileInfo : fileList) {
          ...
          QString formattedName = baseName.replace('_', ' ').toLower();
          formattedName[0] = formattedName[0].toUpper();
        ...
        if (id == 0) {
          ...
          if (!imageToDelete.isEmpty() && ConfirmationDialog::confirm(tr("Are you sure you want to delete the '%1' steering wheel image?").arg(imageToDelete), tr("Delete"), this)) {
            ...
            QString selectedImage = formatWheelName(imageToDelete);
            for (const QFileInfo &fileInfo : fileList) {
          ...
          if (manageWheelIconsBtn->getButton(id)->text() == tr("CANCEL")) {
            ...
            if (!wheelToDownload.isEmpty()) {
              QString convertedImage = formatWheelName(wheelToDownload);
          ...
          if (!imageToSelect.isEmpty()) {
            params.put("WheelIcon", formatWheelName(imageToSelect).toStdString());
            manageWheelIconsBtn->setValue(imageToSelect);
          }
        }
      });
      QString wheelIcon = QString::fromStdString(params.get("WheelIcon")).replace('_', ' ').toLower();
      wheelIcon[0] = wheelIcon[0].toUpper();
      for (int i = 1; i < wheelIcon.length(); ++i) {
        if (wheelIcon[i - 1] == ' ' || wheelIcon[i - 1] == '(') {
          wheelIcon[i] = wheelIcon[i].toUpper();
        }
      }
      manageWheelIconsBtn->setValue(wheelIcon);
