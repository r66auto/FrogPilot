...
def update_wheel_image(image, holiday_theme=None, random_event=True):
  if random_event:
    wheel_locations = os.path.join(BASEDIR, "selfdrive", "frogpilot", "assets", "random_events", "icons")
  elif holiday_theme:
    wheel_locations = os.path.join(BASEDIR, "selfdrive", "frogpilot", "assets", "holiday_themes", holiday_theme, "icons")
  else:
  ...
  wheel_save_location = os.path.join(ACTIVE_THEME_PATH, "icons")
...
class ThemeManager:
  ...
  def update_active_theme(self, frogpilot_toggles, params):
    asset_types = {
      "color_scheme": "colors",
      "icon_pack": "icons",
      "sound_pack": "sounds",
      "turn_signal_pack": "signals",
      "wheel_image": "wheel_image"
    }

    for toggle, asset_type in asset_types.items():
      current_value = getattr(frogpilot_toggles, toggle)
      if current_value != self.previous_assets.get(toggle):
        if asset_type == "wheel_image":
          ...
          copy_theme_asset(asset_type, current_value, frogpilot_toggles.current_holiday_theme, params)
