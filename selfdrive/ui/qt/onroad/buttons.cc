...
ExperimentalButton::ExperimentalButton(QWidget *parent) : experimental_mode(false), engageable(false), QPushButton(parent) {
  ...
  // FrogPilot variables
  wheel_gif_path = "../frogpilot/assets/active_theme/icons/wheel.gif";
  wheel_png_path = "../frogpilot/assets/active_theme/icons/wheel.png";
...
void ExperimentalButton::updateIcon() {
  if (QFile::exists(wheel_gif_path)) {
    if (!use_gif || (gif && gif->fileName() != wheel_gif_path)) {
      if (gif) {
        delete gif;
      }
    ...
    if (QFile::exists(wheel_png_path)) {
...
void DistanceButton::updateState(const UIScene &scene) {
  ...
  if (state_changed) {
    ...
    if (profile_data_gif[profile_index]) {
      gif_label->setMovie(profile_data_gif[profile_index]);
      gif_label->show();

      profile_data_gif[profile_index]->start();

      use_gif = true;
    } else {
      profile_image = profile_data_png[profile_index];
...
void DistanceButton::updateIcon() {
  ...
  for (const QString &file_name : file_names) {
    ...
    if (QFile::exists(gif_file)) {
      ...
      QPixmap pixmap = QPixmap(QFile::exists(png_file) ? png_file : fallback_file).scaled(img_size, img_size, Qt::KeepAspectRatio, Qt::SmoothTransformation);
...
void DistanceButton::paintEvent(QPaintEvent *event) {
  ...
  drawIcon(p, QPoint((btn_size / 2) * 1.25, btn_size), profile_image, Qt::transparent, 1.0);
}
